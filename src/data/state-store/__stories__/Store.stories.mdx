...
-   `prev`: If there is a pending call `prev` will be a promise that fulfills with when the result of the call. There is also an `abort` method defined that will abort the previous call.
-   `signal`: A [AbortSignal](https://developer.mozilla.org/en-US/docs/Web/API/AbortSignal) instance that will indicate when/if this call has been aborted.
...
StateStores have three main lifecyle actions that are invoked automatically by `useStore`.

1. `load` - Given the current state and parameters of the store load the next state(s) of the store.
   A. initialLoad - triggered when `useStore` mounts a new store instance. Will only run if load has not been called before for the instance.
   B. reload - triggered by `setParams` if params changed. Will only run if load has been run at least once before.
